/**
 * ╔══════════════════════════════════════════════════════════╗
 * ║         MAVERICK INTEL BOT v4.1                          ║
 * ║                                                          ║
 * ║  v4.1 FIXES:                                             ║
 * ║  • HTTP server added — Render no longer kills the bot    ║
 * ║  • Yahoo Finance is now PRIMARY data source (free/live)  ║
 * ║  • Polygon snapshot removed — not on free plan           ║
 * ║  • AI intent detection — typing "NIXX" works now         ║
 * ║  • Account size language generalized (no $ amounts)      ║
 * ║                                                          ║
 * ║  SCIENCE MODULE (v4.0):                                  ║
...
function detectIntent(text) {
  ...
  if (/\b(scan|gapper|top mover|movers today|what.s moving)\b/.test(lower)) return 'scan';
  ...
  if (/\b(gap up|gapping|gappers)\b/.test(lower)) return 'gappers';
  return null;
}
...
async function cmdStart(chatId) {
  await tg(
    ...
    'Just type: "What\'s NIXX doing?" or "scan for me" or "$AAPL"\n' +
    'I pull live data and think. No disclaimers.',
    chatId
  );
}
...
async function start() {
  console.log('\n╔══════════════════════════════════════╗');
  console.log('║    MAVERICK INTEL BOT v4.1           ║');
  console.log('╚══════════════════════════════════════╝\n');
  ...
  await loadMemory();
  await tg(
    '<b>MAVERICK INTEL BOT v4.1 — ONLINE</b>\n\n' +
    ...
    '<b>v4.1 FIXES:</b>\n' +
    '• Just type "$NIXX" or "What\'s NIXX doing?" — I pull live data\n' +
    '• Type "scan" — I run the scanner\n' +
    '• Port issue fixed — bot stays alive on Render\n' +
    '• No more stale data from 2022\n\n' +
  ...
  poll();
  console.log('[BOT] v4.1 running. HTTP server active. All intervals live. Alerts will fire proactively.');
}
