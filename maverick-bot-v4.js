...
async function deduped(key, fn) {
  ...
  inFlight[key] = fn().finally(function(){ delete inFlight[key]; });
  return inFlight[key];
}
...
async function fhQuote(sym) {
  ...
  try {
    ...
    var t = await r.text(); if (!t || t.trim() === '') return null;
    var d = JSON.parse(t);
...
async function fh(ep) {
  ...
  try {
    ...
    var t   = await r.text(); if (!t||t.trim()==='') return null; return JSON.parse(t);
  } catch(e) { return null; }
}
...
async function polyNewsRaw(tickerOrNull, limit) {
  ...
  try {
    ...
    var r = await sFetch(url); var d = await r.json(); if (d && d.results) return d.results;
  } catch(e) {}
  return [];
}
...
async function getStock(sym) {
  ...
  try {
    ...
    var result = {
      ...
      // Extended hours (from Yahoo when available)
...
async function cmdFocusChat(text, chatId) {
  // Auto-detect ticker in message
  var words = text.toUpperCase().split(/\s+/);
  var ticker = words.find(function(w){
    return w.length >= 2 && w.length <= 5 && /^[A-Z]+$/.test(w) &&
           !['WHAT','WHEN','WHERE','WHY','HOW','THE','AND','BUT','YOU','FOR','ARE','NOT','ITS','LET','SEE','CAN','GET','BUY','PUT','YES','NOW','ALL','ANY'].includes(w);
  });
...
async function start() {
  console.log('\n╔══════════════════════════════════════╗');
  console.log('║    MAVERICK INTEL BOT v6.3           ║');
  console.log('║    FOCUS+INTRADAY+ALGO-WATCH+EH DATA ║');
  console.log('╚══════════════════════════════════════╝\n');
  ...
  await tg(
    '<b>MAVERICK INTEL BOT v6.3 — ONLINE</b>\n\n' +
    '🎯 Focus Mode | 🤖 Algo Watchlist | 📊 Intraday Scanner | 🌙 After-Hours\n\n' +
    '<b>v6.3 — FULL ARSENAL:</b>\n' +
    '• /focus — expert peer conversation, no training wheels\n' +
    '• /awatch — algo\'s autonomous 7-slot arsenal (updates every 10min)\n' +
    '• /mywatch — your watchlist (separate from algo)\n' +
    '• /intraday — VWAP proxy whale detector + volume pace scanner\n' +
    '• After-hours data live in all outputs when market closed\n' +
    '• Algo watchlist alerts you on every slot change (entry + exit)\n' +
    '• Combined algo+intraday scan shares cache = minimal API calls\n\n' +
    'Type /focus to start talking trade.\nType /awatch to see the arsenal.'
  );
  ...
  setInterval(runAlgoAndIntradayScan, 600000); // Every 10 min — algo watchlist + intraday
  setInterval(warmCache, 1800000);
  setTimeout(warmCache, 45000);
  setTimeout(runAlgoAndIntradayScan, 90000); // First algo-watchlist build 90s after start

  console.log('[BOT] v6.3 running. Focus mode. Algo watchlist. Intraday VWAP scanner. After-hours data. No polling.');
}
