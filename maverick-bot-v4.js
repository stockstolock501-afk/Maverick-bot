...
async function start() {
  console.log('\n╔══════════════════════════════════════╗');
  console.log('║    MAVERICK INTEL BOT v5.7           ║');
  console.log('║    TRIPLE-REDUNDANT DATA LAYER       ║');
  console.log('╚══════════════════════════════════════╝\n');
  ...
  await tg(
    '<b>MAVERICK INTEL BOT v5.7 — ONLINE</b>\n\n' +
    '⚡ Webhook | 🔁 Triple-redundant data | 📦 Tiered cache\n' +
    'Quotes:   Yahoo → Finnhub (60s cache)\n' +
    'History:  Yahoo → Finnhub Candles → Polygon (4hr cache)\n' +
    'Fundament: Finnhub Metrics (24hr cache)\n' +
    'Gainers:  Yahoo Screener (5min cache)\n' +
    'Brain:    '+(GROQ_KEY?'Groq ✓':CBRS_KEY?'Cerebras':'NO AI KEYS')+'\n' +
    'Memory:   '+(JSONBIN_ID?(memory.trades?memory.trades.length:0)+' trades ✓':'not configured')+'\n\n' +
    '<b>v5.7 DATA PILLAR:</b>\n' +
    '• Finnhub Candles = 2nd free source for historical bars\n' +
    '• Aggs cached 4hr — science/sas/supernova now fast\n' +
    '• Float/short% cached 24hr — no repeat calls\n' +
    '• Smart retry: 1 auto-retry before fallback\n' +
    '• Dedup queue: no duplicate simultaneous calls\n' +
    '• Gainers cached 5min — scanner runs fast\n\n' +
    'Run /test-alert SPY to verify pipeline.\nType /status to see live cache health.'
  );
  ...
  console.log('[BOT] v5.7 running. Triple-redundant data. Tiered cache. Webhook. No polling.');
}
