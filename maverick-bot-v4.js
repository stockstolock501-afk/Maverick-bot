...
// ── FINVIZ SHORT INTEREST — Multi-pattern scraper (FINRA biweekly data) ──────
// Finviz changes their HTML periodically — use 5 patterns for resilience.
// Cached 1 hour. Only fires when Finnhub returned 0%.
var finvizShortCache = {}; // ticker → { shortPct, ts }
async function finvizShortFloat(sym) {
  var cached = finvizShortCache[sym];
  if (cached && (Date.now() - cached.ts) < 3600000) return cached.shortPct;
  try {
    var r = await tFetch(
    ...
    if (!html) return 0;
    // Five patterns — first match wins
    var patterns = [
      /Short Float\s*\/\s*Ratio[\s\S]{0,200}?<td[^>]*>([0-9]+\.?[0-9]*)%/i,
      /Short Float[^<]{0,60}<\/td>\s*<td[^>]*>([0-9]+\.?[0-9]*)%/i,
      /Short Float[^%\n]{0,80}?([0-9]+\.[0-9]+)%/i,
      /"shortFloat"\s*:\s*"?([0-9]+\.?[0-9]*)"?/i,
      /data-boxover[^>]*short[^>]*>([0-9]+\.?[0-9]*)%/i
    ];
    var shortPct = 0;
    for (var pi = 0; pi < patterns.length; pi++) {
      var m = html.match(patterns[pi]);
      if (m && parseFloat(m[1]) > 0) { shortPct = parseFloat(m[1]); break; }
    }
    finvizShortCache[sym] = { shortPct: shortPct, ts: Date.now() };
    if (shortPct > 0) console.log('[FINVIZ] $'+sym+' short float: '+shortPct+'%');
    return shortPct;
  } catch(e) { return 0; }
}

// ── SHORTVOLUME.COM — Daily FINRA short volume % (free, no key) ───────────
// Complements Finviz — FINRA reports biweekly but short volume is daily.
// Short volume % > 50% = sustained short pressure on the name.
var shortVolumeCache = {};
async function shortVolumeFloat(sym) {
  var cached = shortVolumeCache[sym];
  if (cached && (Date.now() - cached.ts) < 3600000) return cached.shortPct;
  try {
    var r = await tFetch(
      'https://shortvolume.com/?t='+sym,
      { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' } },
      8000
    );
    var html = await safeText(r);
    if (!html) return 0;
    // Look for "Short Vol %" in table — format: XX.X%
    var m = html.match(/Short Vol[^%]{0,60}?([0-9]+\.?[0-9]*)%/i);
    var shortPct = m ? parseFloat(m[1]) : 0;
    shortVolumeCache[sym] = { shortPct: shortPct, ts: Date.now() };
    if (shortPct > 0) console.log('[SHORTVOL] $'+sym+' short vol: '+shortPct+'%');
    return shortPct;
  } catch(e) { return 0; }
}

// ── STOCKTWITS SENTIMENT — Community bullish/bearish ratio ────────────────
// Not short interest, but a real-time crowd sentiment signal.
// Used as a supplementary input to SDI scoring.
// Bull% > 70: crowd chasing = late stage / sell into strength
// Bull% < 30: max fear = Archetype B dip-buy territory
var stocktwitsCache = {};
async function getStocktwitsSentiment(sym) {
  var cached = stocktwitsCache[sym];
  if (cached && (Date.now() - cached.ts) < 600000) return cached; // 10 min cache
  try {
    var r = await tFetch(
    ...
    if (!d || !d.messages || !d.messages.length) return null;
    var bull = 0, bear = 0;
    d.messages.slice(0, 30).forEach(function(msg) {
      var s = msg.entities && msg.entities.sentiment && msg.entities.sentiment.basic;
      if (s === 'Bullish') bull++;
      else if (s === 'Bearish') bear++;
    });
    var total = bull + bear;
    if (total < 3) return null; // not enough signal
    var bullPct = rnd(bull / total * 100, 1);
    var result = { bullPct: bullPct, bearPct: rnd(100 - bullPct, 1), total: total, ts: Date.now() };
    stocktwitsCache[sym] = result;
    console.log('[STOCKTWITS] $'+sym+' sentiment: '+bullPct+'% bull ('+total+' msgs)');
    return result;
  } catch(e) { return null; }
}

// ── UNIFIED SHORT INTEREST — 3-source waterfall ───────────────────────────
// 1. Finnhub metrics (already in getStock)
// 2. Finviz (multi-pattern)
// 3. ShortVolume.com
// Returns best non-zero value found.
async function getShortInterest(sym, price) {
  if (!price || price > TRADE_MAX_PRICE || price < 0.25) return 0;
  // Finviz first (FINRA-sourced, most accurate)
  var fv = await finvizShortFloat(sym).catch(function(){return 0;});
  if (fv > 0) return fv;
  // ShortVolume.com fallback
  var sv = await shortVolumeFloat(sym).catch(function(){return 0;});
  return sv;
}
...
async function getStock(sym) {
  ...
  try {
    ...
    // ── UNIFIED SHORT INTEREST — 3-source waterfall ──────────────────────────
    // Finnhub free tier returns 0% on most small-caps → fall through to Finviz/ShortVol
    if (shortPct === 0 && price <= TRADE_MAX_PRICE && price >= 0.25) {
      try {
        var unifiedShort = await getShortInterest(sym, price);
        if (unifiedShort > 0) { shortPct = unifiedShort; }
      } catch(e) {}
    }
...
async function runPreMarketScanner() {
  ...
  for (var i=0; i<universe.length; i++) {
    ...
    try {
      ...
      // ── WASH TRADING FILTER — skip algo-spoofed pre-market volume ─────────
      var sasPM = calcSAS(d);
      var isWash = (d.relVol > 5 || (d.floatRotPct && d.floatRotPct > 150)) && sasPM.score < 30;
      if (isWash) { console.log('[PRE-MKT] $'+sym+' — WASH SUSPECTED (RVOL:'+d.relVol+'x SAS:'+sasPM.score+'). Skipping.'); continue; }
...
function scoreSetup(d) {
  ...
  // High volume + low SAS = algo spoofing. Volume without accumulation = trap.
  var sasCheck = calcSAS(d);
  var floatRotHigh = (d.floatRotPct && d.floatRotPct > 150);
  var highVolLowSAS = (d.relVol > 5 || floatRotHigh) && sasCheck.score < 35;
  if (highVolLowSAS) { score -= 15; flags.push('🚨 ALGO SPOOFING SUSPECT — high volume, low accumulation signal'); }

  return { score: Math.min(100, Math.max(0, score)), flags, country, zone: zone.label, washWarning: highVolLowSAS };
}
...
async function cmdFocusChat(text, chatId) {
  ...
  if (ticker) {
    ...
    if (d) {
      ...
      // Inject StockTwits sentiment if available
      try {
        var stFocus = await getStocktwitsSentiment(ticker).catch(function(){return null;});
        if (stFocus && stFocus.total >= 3) {
          liveCtx += 'StockTwits: '+stFocus.bullPct+'% bull / '+stFocus.bearPct+'% bear ('+stFocus.total+' msgs)\n';
        }
      } catch(e) {}
      liveCtx +=
...
async function scanNewsIntel() {
        ...
        try {
          ...
          for (var tn=0; tn<Math.min(fresh.length,3); tn++) {
            ...
            if (isBearish) {
              ...
              // ── INVERSE CATALYST: watch for short covering on bad news ──
              var invD = await getStock(sym).catch(function(){return null;});
              if (invD) {
                var invSDI = calcSDI(invD, 5);
                if (invSDI.score >= 55) { // Enough short fuel to create inverse squeeze
                  inverseCatalystWatch[sym] = { firedAt: Date.now(), openPrice: invD.open || invD.price, sdi: invSDI.score };
                  console.log('[INVERSE] Watching $'+sym+' SDI:'+invSDI.score+' for price-holds-open pattern');
                }
              }
            }
          }
  ...
  else console.log('[NEWS] Scan complete — no new catalysts found. Total alerts ever: ' + iHealth.newsAlertsTotal + ' | Sources checked: Finnhub/EDGAR/RSS/GNW/Yahoo/Reuters');
}

// ── MORNING BRIEFING ───────────────────────────────────────────────────────
// Fires at EXACTLY 4 time slots per day (CT):
//   7:00AM  — overnight summary + pre-market movers
//   8:00AM  — live pre-market float rotation + gappers
//   9:20AM  — final pre-open brief (10 min before bell)
//   2:45PM  — afternoon positioning (optional)
// checkBriefingSlots() runs every 60s and gates against a fired-today set.
var briefingSlotsToday = {}; // { date: { '700':bool, '800':bool, '920':bool, '1445':bool } }

function checkBriefingSlots() {
  var today = todayStr();
  if (!briefingSlotsToday[today]) briefingSlotsToday[today] = {};
  var slots = briefingSlotsToday[today];
  var t = nowTimeCT();
  // Clean yesterday's slot state
  Object.keys(briefingSlotsToday).forEach(function(d){ if(d!==today) delete briefingSlotsToday[d]; });

  if (t >= 7.0 && t < 7.1 && !slots['700']) {
    slots['700'] = true;
    morningBriefing(false).catch(function(e){ console.error('[BRIEF-700]', e.message); });
  } else if (t >= 8.0 && t < 8.1 && !slots['800']) {
    slots['800'] = true;
    morningBriefing(false).catch(function(e){ console.error('[BRIEF-800]', e.message); });
  } else if (t >= 9.333 && t < 9.433 && !slots['920']) {
    // 9:20 = 9 + 20/60 = 9.333
    slots['920'] = true;
    morningBriefing(false).catch(function(e){ console.error('[BRIEF-920]', e.message); });
  } else if (t >= 14.75 && t < 14.85 && !slots['1445']) {
    // 2:45PM = 14.75
    slots['1445'] = true;
    morningBriefing(false).catch(function(e){ console.error('[BRIEF-1445]', e.message); });
  }
}
...
async function morningBriefing(manual) {
  ...
  // When called from checkBriefingSlots, manual=false but we always proceed — slot gating already done above.
  // Manual /briefing command also always proceeds.
...
async function cmdCheck(sym, chatId) {
  ...
  var vwapSniper = calcVWAPSniper(d, mis.pct); // VWAP mean-reversion sniper
...
async function cmdSqueeze(chatId) {
  ...
  var universe = Object.keys(watchlist).concat(BASE_SCAN).filter(function(v,i,a){return a.indexOf(v)===i;}).slice(0,25);
  var settled  = await Promise.allSettled(universe.map(function(s){return getStock(s);}));
  var results  = [], uncertain = [];

  for (var i=0; i<settled.length; i++) {
    var r = settled[i];
    if (r.status!=='fulfilled'||!r.value) continue;
    var dq = r.value;
    if (dq.price > TRADE_MAX_PRICE || dq.price < 0.25) continue;
    var sdi = calcSDI(dq, 5);
    if (dq.shortPct === 0) {
      // Cannot confirm short interest — exclude from main ranking, flag separately
      if (sdi.score >= 40) uncertain.push(Object.assign({}, dq, {sqSc: sdi.score}));
      continue;
    }
    if (sdi.score >= 30 || dq.shortPct >= 10) results.push(Object.assign({}, dq, {sqSc: sdi.score}));
  }

  results.sort(function(a,b){ return b.sqSc - a.sqSc; });
  uncertain.sort(function(a,b){ return b.sqSc - a.sqSc; });

  var msg = '<b>🗜️ SQUEEZE SCAN (SDI-Powered)</b>\n\n';

  if (results.length) {
    msg += '<b>CONFIRMED SHORT DATA:</b>\n';
    for (var j=0; j<Math.min(5, results.length); j++) {
      var d2=results[j], danger=d2.sqSc>=75?'🔥 EXTREME':d2.sqSc>=55?'⚡ HIGH':'👀 WATCH';
      msg += danger+' <b>$'+d2.sym+'</b> '+cFlag(d2.sym)+' SDI:'+d2.sqSc+'/100\n';
      msg += 'Short:'+rnd(d2.shortPct,1)+'%  RVOL:'+d2.relVol+'x  Float:'+d2.floatM+'M  DTC:'+d2.daysToCover+'d\n';
      msg += '/cascade '+d2.sym+'\n\n';
    }
  }

  if (uncertain.length) {
    msg += '<b>⚠️ SHORT DATA UNCERTAIN (0% reported):</b>\n';
    msg += '<i>These may have squeeze potential but short interest unconfirmed via free data. Verify manually.</i>\n';
    uncertain.slice(0,3).forEach(function(d3){
      msg += '$'+d3.sym+' '+cFlag(d3.sym)+' SDI:'+d3.sqSc+' RVOL:'+d3.relVol+'x — confirm short % before trading\n';
    });
    msg += '\n';
  }

  if (!results.length && !uncertain.length) return tg('No notable squeeze setups detected. Universe: '+universe.length+' tickers.', chatId);
  msg += 'SDI > 75 = EXTREME short danger. /cascade TICKER for full cascade analysis.';
  await tg(msg, chatId);
}
...
async function monitorPositions() {
  ...
  for(var sym in positions){
    ...
    // ── LULD HALT RISK DETECTION ─────────────────────────────────────────────
    // Price velocity 15%+ in one 60-second monitor cycle = approaching LULD threshold.
    // Fire warning BEFORE halt traps you with a market stop open.
    if (!pos._lastPrice) { pos._lastPrice = price; }
    else {
      var priceDelta = Math.abs((price - pos._lastPrice) / Math.max(pos._lastPrice, 0.01) * 100);
      if (priceDelta >= 15 && d.relVol >= 8 && !pos.alerts.haltWarn) {
        pos.alerts.haltWarn = true;
        setTimeout(function(){ if(positions[sym]) delete positions[sym].alerts.haltWarn; }, 300000);
        await tg(
          '🚨 <b>HALT RISK — $'+sym+'</b>'+marketLabel+'\n\n'+
          'Price moved '+rnd(priceDelta,1)+'% in 60 seconds. RVOL: '+d.relVol+'x.\n'+
          'LULD volatility halt may be imminent.\n\n'+
          '<b>Action required NOW:</b>\n'+
          '• Switch any MARKET STOP to a LIMIT STOP with a floor\n'+
          '• Market stops execute at reopening price — often 10-15% below halt level\n'+
          '• Limit stop: set stop trigger at $'+pos.stop+', floor at $'+rnd(pos.stop*0.93,4)+'\n\n'+
          'A halt is not a reason to panic. It IS a reason to be prepared.'
        );
      }
      pos._lastPrice = price;
    }

    // ── INVERSE CATALYST CHECK ────────────────────────────────────────────────
    if (inverseCatalystWatch[sym]) { await checkInverseCatalyst(sym); }
  }
}
...
function calcVWAPSniper(d, misScore) {
  ...
  var aboveVwap  = d.price >= vwapProxy;
  var vwapDevPct = rnd((d.price - vwapProxy) / vwapProxy * 100, 2);
  var isSniperEntry = !aboveVwap && Math.abs(vwapDevPct) >= 8 && (misScore||0) >= 60 && d.relVol >= 3;
  return { vwapProxy, aboveVwap, vwapDevPct, isSniperEntry,
    sniperNote: isSniperEntry
      ? '🎯 VWAP SNIPER ENTRY: Price '+Math.abs(vwapDevPct)+'% below VWAP ($'+vwapProxy+'). Institutional execution bots are FORCED to accumulate here. Rubber-band distortion — snap-back probable.'
      : null
  };
}

// ── INVERSE CATALYST CIRCUIT ──────────────────────────────────────────────
var inverseCatalystWatch = {};
async function checkInverseCatalyst(sym) {
  var watch = inverseCatalystWatch[sym];
  if (!watch) return;
  var elapsed = (Date.now() - watch.firedAt) / 60000;
  if (elapsed < 5 || elapsed > 45) { delete inverseCatalystWatch[sym]; return; }
  var d = await getStock(sym).catch(function(){return null;});
  if (!d) return;
  if (d.price >= watch.openPrice * 0.98) {
    var msg = '🔄 <b>INVERSE CATALYST — $'+sym+' '+cFlag(sym)+'</b>\n\n';
    msg += 'Bearish news fired '+Math.round(elapsed)+'min ago. <b>Price held above open. Shorts covering.</b>\n';
    msg += 'SDI: '+watch.sdi+'/100 — shorts were trapped BEFORE the news.\n';
    msg += 'Current: $'+d.price+'  Open: $'+watch.openPrice+'\n\n';
    msg += '<b>Asymmetric long entry: the bad news IS the exit for trapped shorts.</b>\n';
    msg += 'Entry: $'+rnd(d.price*0.99,4)+'–$'+rnd(d.price*1.005,4)+'\n';
    msg += 'Stop: $'+rnd(d.price*0.94,4)+' (below catalyst low)\n\n';
    msg += '/cascade '+sym+' | /science '+sym;
    await tg(msg); organicAlertsSent++;
    delete inverseCatalystWatch[sym];
  }
}
...
async function start() {
  ...
  await tg(
    '<b>⚔️ MAVERICK INTEL BOT v7.1 — ONLINE</b>\n\n' +
    '📡 6 News Sources | 📍 Cluster Engine | 📊 3-Source Short Intel | 🔥 Genome Calibrated\n' +
    '💬 StockTwits Sentiment | 🕐 Precision Briefings: 7AM / 8AM / 9:20AM / 2:45PM CT\n\n' +
    '<b>SHORT INTEREST STACK:</b>\n' +
    '• Finnhub metrics (primary)\n' +
    '• Finviz multi-pattern scraper (FINRA biweekly)\n' +
    '• ShortVolume.com (daily FINRA short vol %)\n\n' +
    ...
    '<b>v7.1 ENGINES:</b> Cluster Map | Flush Absorption | FPR | Cascade | Confidence Pulse | StockTwits\n\n' +
  ...
  setInterval(checkBriefingSlots, 60000); // ← Fires ONLY at 7:00, 8:00, 9:20, 14:45 CT
